...
class MinimalEKF:
    def __init__(self):
        ...
        # ตั้งค่า initial velocity ให้เป็น 1.0 m/s เพื่อให้พฤติกรรมคล้ายกับเดิม
        self.x_est = np.array([0.0, 0.0, 0.0, 1.0])  
        self.P_est = np.eye(4) * 1.0            # initial covariance matrix

        # Process noise covariance matrix (4x4)
        self.Q = np.diag([0.015, 0.015, 0.015, 0.015])
        
        # Measurement noise covariance matrixสำหรับ GPS (2x2)
